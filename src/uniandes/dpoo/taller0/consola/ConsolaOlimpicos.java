...
/**
 * Este es el método que creé yo para entregar el país de un atleta dado por el usuario.
 */
	

	private void ejecutarEntregarPais()
	{
		System.out.println("\n" + "Nombre de atleta cuyo país desea consultar" + "\n");

		String atleta = input("Por favor ingrese el nombre de un atleta");
		Atleta atletaExistente = calculadora.buscarAtleta(atleta);
		if (atletaExistente == null)
			{
			System.out.println("Atleta no encontrado/a");
			}
		else {
			if (atletaExistente.darGenero()==Genero.MASCULINO)
				{
					System.out.println("El atleta "+atletaExistente.darNombre()+" es de "+atletaExistente.darPais().darNombre());
				}
			else {
					System.out.println("La atleta "+atletaExistente.darNombre()+" es de "+atletaExistente.darPais().darNombre());
				}
			}
		}
